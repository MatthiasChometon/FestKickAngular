...
export class ConcertListComponent implements AfterViewInit {
  ...
  constructor(private songKickApi: SongKickApiService, private route: ActivatedRoute, private router: Router) {
    //récupére le nom de la ville tapée dnas l'input de la mainPage
    this.route
      .queryParams
      .subscribe(params => {
  ...
  getCityConcert(city) {
    //récupère les informations des concerts en fonction du nom de la ville
    if (city) {
  ...
  displayMap() {
    //active ou désactive la map, puis, je récupère les données de localisation du user pour récupérer les concerts de sa ville
  ...
  localisation(): void {
    //récupère les coordonnées du user
  ...
  createMap(concerts) {
    //crée la map et les markers associés
